...
@Composable
private fun WellnessLineChart(
    data: List<DailyCheckIn>,
    modifier: Modifier = Modifier
) {
    ...
    val axisLabelPaint = remember(density) {
        android.graphics.Paint().apply {
            color = android.graphics.Color.rgb(122, 96, 128) // TextMuted #7A6080
    ...
    Canvas(modifier = modifier) {
        ...
        for (i in 0 until n - 1) {
            drawLine(
                color = SleepPurple,
                start = Offset(xFor(i),     sleepY(data[i].sleepHours)),
                end   = Offset(xFor(i + 1), sleepY(data[i + 1].sleepHours)),
        ...
        for (i in 0 until n - 1) {
            drawLine(
                color = EnergyTeal,
                start = Offset(xFor(i),     energyY(data[i].energyLevel)),
                end   = Offset(xFor(i + 1), energyY(data[i + 1].energyLevel)),
                strokeWidth = 2.dp.toPx(),
                cap = StrokeCap.Round
            )
        }
        ...
        data.forEachIndexed { i, entry ->
            ...
            drawCircle(
                color  = SleepPurple,
                radius = 4.dp.toPx(),
                center = Offset(x, sleepY(entry.sleepHours))
            )

            val energyColour = if (entry.energyLevel < 4) WarningRed else EnergyTeal
            if (entry.energyLevel < 4) {
                drawCircle(
                    color  = WarningRed.copy(alpha = 0.25f),
                    radius = 9.dp.toPx(),
                    center = Offset(x, energyY(entry.energyLevel))
                )
            }
            drawCircle(
                color  = energyColour,
